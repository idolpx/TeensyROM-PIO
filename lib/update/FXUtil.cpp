...
int process_hex_record ( hex_info_t *hex )
{
    if (hex->code == 0) // data -- update min/max address so far
    {
        ...
        if (hex->code == 1) // EOF (:flash command not received yet)
        {
            hex->eof = 1;
        }
        else
            if (hex->code == 2) // extended segment address (top 16 of 24-bit addr)
            {
                hex->base = ((hex->data[0] << 8) | hex->data[1]) << 4;
            }
            else
                if (hex->code == 3) // start segment address (80x86 real mode only)
                {
                    return 1;
                }
                else
                    if (hex->code == 4) // extended linear address (top 16 of 32-bit addr)
                    {
                        hex->base = ((hex->data[0] << 8) | hex->data[1]) << 16;
                    }
                    else
                        if (hex->code == 5) // start linear address (32-bit big endian addr)
                        {
